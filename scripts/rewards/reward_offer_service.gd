...
func claim_option_in_transaction(
	...
	elif offer.rule == GameEnums.RewardOfferRule.PURCHASE_ANY:
...
func skip_option(
	...
	var result: RunCommandResult = skip_option_in_transaction(
		working,
		offer_id,
		option_id
	)
...
func skip_option_in_transaction(
	run: RunState,
	offer_id: int,
	option_id: int
) -> RunCommandResult:
	...
	if not validation.succeeded():
		return validation
	if offer.rule != GameEnums.RewardOfferRule.PICK_ANY:
		return RunCommandResult.failure(
				GameEnums.RunCommandCode.OPTION_UNAVAILABLE
		)
	var option: RewardOption = offer.get_option(option_id)
	...
	option.status = GameEnums.RewardOptionStatus.SKIPPED
	return RunCommandResult.success()


func finish_offer(run: RunState, offer_id: int) -> RunCommandResult:
	var transaction: RunTransaction = RunTransaction.begin(run)
	if transaction == null or transaction.working_state == null:
		return RunCommandResult.failure(
				GameEnums.RunCommandCode.INVALID_RUN
	...
	var result: RunCommandResult = finish_offer_in_transaction(
		working,
		offer_id
	)
	if not result.succeeded():
...
func finish_offer_in_transaction(
	run: RunState,
	offer_id: int
) -> RunCommandResult:
	if run == null:
